...
export default function PinModal({ onSuccess, onClose }) {
  ...
  return (
    <div
      style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, backdropFilter: 'blur(4px)' }}
      onClick={onClose}
    >
      <div
        onClick={e => e.stopPropagation()}
        style={{ background: '#fff', borderRadius: 28, padding: '36px 32px', width: 320, boxShadow: '0 24px 80px rgba(0,0,0,0.25)', animation: shake ? 'shake 0.5s' : undefined }}
      >
        ...
        {/* Keypad — onClick only to avoid double-fire on mobile */}
        ...
        {error && (
          <div style={{ textAlign: 'center', color: '#dc2626', fontSize: 14, fontWeight: 600, marginBottom: 12 }}>{error}</div>
        )}
